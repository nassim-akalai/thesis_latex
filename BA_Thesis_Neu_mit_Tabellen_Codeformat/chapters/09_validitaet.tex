...
Dieses Kapitel bewertet die Aussagekraft und die Grenzen der durchgef{\"u}hrten Untersuchung. Dabei wird betrachtet, wie sehr die beobachteten Unterschiede auf die verschiedenen Zephyr-Konfigurationen zur{\"u}ckgef{\"u}hrt werden k{\"o}nnen und unter welchen Bedingungen die Ergebnisse auf andere Systeme {\"u}bertragbar sind. Dar{\"u}ber hinaus wird der Einfluss des Messverfahrens selbst und weitere methodische Einschr{\"a}nkungen erl{\"a}utert. Die Betrachtung erfolgt dabei getrennt nach interner Validit{\"a}t, externer Validit{\"a}t, messbedingtem Zusatzaufwand und den allgemeinen Limitierungen der Untersuchung.

\section{Interne Validit{\"a}t}
Um die interene Validit{\"a}t zu verbessern wurden Hardware, Taktfrequenz, Zephyr-Version, Werkzeugkette, Optimierungsstufe, Messcode, Wiederholungszahl und Messverfahren konstant gehalten. Die drei Profile bauen auf demselben gemeinsamen Konfigurationssockel auf. Au{\ss}erdem wurde jede Kombination aus Profil und Benchmark als eigenes Firmware-Abbild erstellt.

Dennoch k{\"o}nnen {\"A}nderungen der Konfiguration das erzeugte Speicherlayout ver{\"a}ndern. Vor allem bei sehr kurzen Messpfaden k{\"o}nnen dadurch Flash-Ausrichtung, Prefetch und benachbarte Instruktionen das Ergebnis beeinflussen. Dies begrenzt die M{\"o}glichkeit, kleine Unterschiede genau einem bestimmten Subsystem zuzuordnen.

Die f{\"u}r OpenThread erforderlichen gr{\"o}{\ss}eren Stacks stellen dabei keinen unbeabsichtigten St{\"o}rfaktor dar. Ohne diese Anpassung war das IoT-Profil nicht lauff{\"a}hig. Ihr Speicherbedarf wird daher bewusst dem kompletten IoT-Profil zugerechnet.
...
In Kapitel 8.4 wurde eine Einordnung dieser Arbeit gegen Silva et al. (2019) durchgef{\"u}hrt, welche einen externen Referenzpunkt bringt. Wenn die eigenen Werte des Speicherbedarfs mit denen einer unabh{\"a}ngigen Studie {\"u}bereinstimmen oder sich {\"a}hneln, dann wird das Vertrauen in die {\"U}bertragbarkeit der Methodik dadurch verst{\"a}rkt. Es ersetzt jedoch auf der anderen Seite keine eigene Messung auf anderer Hardware oder mit anderen Zephyr-Versionen.
...
Pro Kombination aus Profil und Metrik wurde eine zusammenh{\"a}ngende Messreihe mit 1000 Werten innerhalb von einem Bootvorgang aufgenommen. Die Standardabweichung beschreibt daher nur die Streuung innerhalb von diesem Lauf. Eine Streuung zwischen unabh{\"a}ngigen Builds, Flash-Vorg{\"a}ngen und Neustarts wurde nicht bestimmt. Da nur zusammengefasste Kennzahlen ausgegeben werden, k{\"o}nnen Verteilungsform, Quantile und einzelne Ausrei{\ss}er nachtr{\"a}glich nicht untersucht werden.

Das Logging-Profil erzeugt w{\"a}hrend der Benchmarks keine Log-Nachrichten. Zudem f{\"u}hrt das IoT-Profil keinen Netzwerkbeitritt, CoAP-Datenaustausch oder DTLS-Handshake aus. Gemessen werden daher die Auswirkungen der bereitgestellten Softwareprofile, nicht die Laufzeitbelastung ihrer aktiven Nutzung.

Der Timer-Benchmark kann Abweichungen unterhalb eines RTC-Zyklus von ungef{\"a}hr 30,52 \textmu{}s nicht erfassen. Auch der Fragmentierungstest liefert keine komplette interne Speicherstatistik des Allokators. Die ausgegebenen Nutzbytes und die Probe im 8-Byte-Raster stellen hierbei nur eine praktische Beschreibung des untersuchten Heapzustands dar.
